...
- Windows 수동 QA 체크리스트와 배포 문서를 현재 자동 CSV 저장·로그 경로에 맞게 갱신
...
> `v26.05.3`은 2026년 5월의 3번째 정식 Release 버전을 의미한다.

### 주요 변경사항 요약

#### fix: 전원 참고 정보 CSV 출력 정리
- `powercfg /a` 전체 원문이 한 CSV 셀에 `/`로 길게 이어져 저장되던 표시 방식을 개선
  - 절전 상태별로 `절전 상태 - ...` 항목을 분리
  - 각 상태의 `사용 가능` / `사용 불가: 사유`를 별도 값으로 기록
- “장치 전원 끄기 허용 요약” 값이 `허용:  / 허용 안 함:`처럼 비어 보이던 PowerShell 문자열 보간 문제 수정
  - `$allowCount개` / `$denyCount개`가 잘못된 변수명으로 해석되던 문제를 `${allowCount}개` / `${denyCount}개` 형태로 보정

#### fix: 자동 점검 진행률 체감 개선
- 전원/절전 참고 정보 수집 추가 후 시스템 정보 단계가 길어진 점을 반영해 자동 점검 진행률 구간 조정
  - 시스템 정보 수집 단계 상한을 32% → 68%로 확대
  - 이후 가상화 점검, 설치 프로그램 수집, CSV 저장 구간을 재배분
- 시스템 정보 수집 중 32%에서 멈춘 것처럼 보이다가 갑자기 완료되는 현상 완화
...
> 버전 표기는 날짜가 아니라 `연도.월.n번째 버전` 규칙을 따른다.
> `v26.04.13`은 2026년 4월의 13번째 정식 Release 버전을 의미한다.

### 주요 변경사항 요약

#### 자동 점검 실패 항목 건너뛰기 및 표시 개선
- 앱 시작 자동 점검 중 일부 수집 단계가 실패해도 전체 점검 흐름이 중단되지 않도록 개선
  - 시스템 정보 수집 실패
  - 가상화 점검 실패
  - 설치된 프로그램 목록 수집 실패
- 실패한 단계는 CSV에 실패 항목으로 기록하고, 완료 화면에는 `건너뛴 점검 항목`으로 표시
- 점검 실패가 있어도 가능한 나머지 항목은 계속 수집하고 CSV 자동 저장 흐름을 유지

#### 시작 점검 완료 화면의 조치 요약 문구 개선
- 개별 레지스트리 항목명이 바로 노출되어 사용자가 오해할 수 있던 완료 화면 요약을 그룹 기준으로 변경
  - 예: `취약 드라이버 차단: 비활성화가 필요합니다`
  - 변경 후: `코어 격리 관련 조치 필요 항목 1건`
- 상단의 `조치 대상 N건 확인`은 실제 조치 필요 항목 수 기준으로 유지
- 상세 항목명은 운영 확인을 위해 가상화 상세 패널과 CSV에는 계속 기록

#### f_im.dll 보안/DRM 후킹 모듈 진단 추가
- 시스템 정보 수집 시 `f_im.dll` 존재 및 서명 상태를 확인하도록 추가
  - `C:\Windows\System32\f_im.dll`
  - `C:\Windows\SysWOW64\f_im.dll`
- 수집 정보
  - 회사명
  - 제품명
  - 파일 버전
  - Authenticode 서명 상태
  - 서명자 정보
- 서명 상태가 정상(`Valid`)이 아닌 경우 `보안 모듈 경고 / f_im.dll` 항목을 시스템 정보 CSV에 기록

#### 프론트엔드 의존성 보안 업데이트
- `postcss` transitive dependency를 `8.5.8`에서 `8.5.12`로 갱신
- `npm audit --audit-level=moderate` 기준 취약점 0건 확인

---

## [Release-v26.04.12] - 2026-04-27

> 버전 표기는 날짜가 아니라 `연도.월.n번째 버전` 규칙을 따른다.
> `v26.04.12`는 2026년 4월의 12번째 정식 Release 버전을 의미한다.

### 주요 변경사항 요약

#### 개선된 조치 GUI에서 기본 조치 항목 선택 지원
- 점검 완료 화면에서 `조치 시작`을 누르면 텍스트형 비활성화 패널을 거치지 않고 개선된 전체화면 조치 GUI로 바로 이동하도록 수정
- 조치 GUI의 warning 단계에서 기본 조치 항목을 체크박스로 선택/해제할 수 있도록 개선
  - Hyper-V 및 하이퍼바이저 비활성화
  - WSL 기능 비활성화
  - VBS 레지스트리 비활성화
  - 코어 격리 비활성화
- 점검 결과 기반 기본 선택값은 유지하되, 사용자가 실제 실행 전 조치 범위를 직접 조정할 수 있도록 변경
- 추가 선택 가능한 레지스트리 조치도 같은 GUI 안에서 체크박스로 선택 후 함께 실행 가능
- 선택된 기본 조치 수와 추가 레지스트리 조치 선택 현황을 GUI 상단 요약에 표시

---

## [Release-v26.04.11] - 2026-04-27

> 버전 표기는 날짜가 아니라 `연도.월.n번째 버전` 규칙을 따른다.
> `v26.04.11`은 2026년 4월의 11번째 정식 Release 버전을 의미한다.

### 주요 변경사항 요약

#### 비활성화 조치 전/후 비교 CSV 추가
- 조치 시작 버튼으로 비활성화 작업을 실행한 뒤 `vmc_logs` 폴더에 전/후 비교 CSV를 자동 저장
  - 파일명: `YYMMDD_HHMMSS_{HostName}-DisableResult.csv`
- CSV 컬럼 추가
  - 작업 그룹
  - 항목
  - 대상
  ...
  - 메시지
- 비교 CSV 기록 대상
  - Hyper-V 관련 DISM Feature 상태
  - WSL 관련 DISM Feature 상태
  - `bcdedit hypervisorlaunchtype`
  - `bcdedit vsmlaunchtype`
  - VBS 레지스트리 조치 대상
  - 코어 격리 레지스트리 조치 대상
  - 사용자가 선택한 추가 레지스트리 조치 대상
- 조치 완료 모달의 저장된 파일 목록에 전/후 비교 CSV 파일명을 함께 표시
- 재부팅 전까지 값이 즉시 바뀌지 않는 항목도 현재 확인된 전/후 상태를 그대로 기록하도록 처리
